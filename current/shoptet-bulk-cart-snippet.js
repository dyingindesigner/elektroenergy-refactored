...
(function shoptetBulkCartNative() {
  ...
  function shouldRenderBulk() {
    ...
    if (RENDER_MODE === "cart-only") {
      return path === "/kosik" || path === "/cart";
    }
    // default: all-except-home (shows on category, product, cart, etc.)
  ...
  async function validateImportedCodes(mergedEntries, onProgress) {
    const invalidCodes = [];
    let lookupErrors = 0;
    const chunkSize = 60;
    const maxToValidate = 1200;
    ...
    for (let i = 0; i < toValidate.length; i += 1) {
      ...
      if ((i + 1) % chunkSize === 0) {
        renderDraftList();
        if (typeof onProgress === "function") onProgress(i + 1, toValidate.length);
      }
      await sleep(15);
    }
    renderDraftList();
    if (typeof onProgress === "function") onProgress(toValidate.length, toValidate.length);
    return {
      invalidCodes,
      lookupErrors,
...
`;
    ...
    draftItems.forEach((it) => {
      ...
      const note =
        it.invalid && it.suggestion
          ? `<div class="bulk-row-note">Kód je pravdepodobne nesprávny.</div><div class="bulk-row-suggest">Nemysleli ste náhodou <button type="button" data-act="use-suggest">${it.suggestion.code}</button>?<span class="sep">|</span><button type="button" data-act="use-suggest-all">Použiť pre všetky podobné</button><span class="sep">|</span><button type="button" data-act="retry-row">Overiť znova</button><span class="sep">|</span><button type="button" data-act="remove-row">Odstrániť</button></div>`
          : it.invalid
            ? '<div class="bulk-row-note">Kód je pravdepodobne nesprávny.</div><div class="bulk-row-suggest"><button type="button" data-act="retry-row">Overiť znova</button><span class="sep">|</span><button type="button" data-act="remove-row">Odstrániť</button></div>'
            : "";
      row.innerHTML = `
      ...
      const retryBtn = row.querySelector('[data-act="retry-row"]');
      if (retryBtn) {
        retryBtn.addEventListener("click", async () => {
          setLog(`Overujem kód ${it.code}...`);
          try {
            const meta = await resolveCodeToMeta(it.code);
            if (meta && meta.status === "exact" && meta.item) {
              it.title = meta.item.title || it.title;
              it.href = meta.item.href || it.href;
              it.img = meta.item.img || it.img;
              it.unitPrice = normalizeUnitPriceText(meta.item.unitPrice || it.unitPrice);
              it.avail = meta.item.avail || it.avail;
              it.stockCount = typeof meta.item.stockCount === "number" ? meta.item.stockCount : it.stockCount;
              it.resolved = true;
              it.invalid = false;
              it.suggestion = null;
              setLog(`Kód ${it.code} bol úspešne overený.`);
            } else {
              it.invalid = true;
              it.resolved = false;
              it.suggestion = meta && meta.suggestion ? meta.suggestion : null;
              setLog(`Kód ${it.code} ostáva neplatný.`);
            }
            renderDraftList();
          } catch (_e) {
            setLog(`Kód ${it.code} sa nepodarilo overiť (sieť/timeout).`);
          }
        });
      }
      const removeRowBtn = row.querySelector('[data-act="remove-row"]');
      if (removeRowBtn) {
        removeRowBtn.addEventListener("click", () => {
          draftItems = draftItems.filter((x) => x.id !== it.id);
          renderDraftList();
          setLog(`Odstránený riadok s kódom ${it.code}.`);
        });
      }
  ...
  async function handleImportFileChange(e) {
    ...
    try {
      ...
      if (!entries.length) {
        ...
        const verify = await validateImportedCodes(merged, (done, total) => {
          if (!total) return;
          const pct = Math.round((done / total) * 100);
          setLog(`Overujem kódy... ${done}/${total} (${pct}%)`);
        });
  ...
  searchInputEl.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      e.preventDefault();
      const focused = searchResultsEl.querySelector(".bulk-result:focus");
      if (focused) {
        focused.click();
      } else {
        doSearch(searchInputEl.value);
    ...
    const resultButtons = Array.from(searchResultsEl.querySelectorAll(".bulk-result"));
    if (!resultButtons.length) return;
    if (e.key === "ArrowDown") {
      e.preventDefault();
      searchActiveIndex = Math.min(resultButtons.length - 1, searchActiveIndex + 1);
      resultButtons[searchActiveIndex].focus();
