(function () {
    ...
    function syncFromOriginalIfAvailable() {
        ...
        if (active && active.id) STATE.lastFocused = active.id;
        var oldSelect = getRetailSelect(form);
        if (oldSelect) STATE.lastCountryValue = oldSelect.value;
        ...
        reinitPhoneFlags(retailGroup);
        if (STATE.lastCountryValue) {
            var newSelect = getRetailSelect(form);
            if (newSelect) newSelect.value = STATE.lastCountryValue;
        }
        if (STATE.lastFocused) {
            var focusTarget = document.getElementById(STATE.lastFocused);
    ...
    function run() {
        var form = getForm();
        if (!form) return;
        STATE.phase = "sync_options";
        ensureRetailGroup();
        syncFromOriginalIfAvailable();
        STATE.phase = "apply_mode";
        syncRetailToOriginal();
        toggleMode();
        STATE.phase = "bind_events";
        bindOnce(form);
        STATE.phase = "ready";
    }

    function observeForm(form) {
        if (!form || STATE.observer) return;
        var rerun = core.debounce ? core.debounce(run, 140) : run;
        STATE.observer = new MutationObserver(function () {
            rerun();
        });
        STATE.observer.observe(form, {
            childList: true,
            subtree: true,
            attributes: true,
