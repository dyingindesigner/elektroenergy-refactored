...
(function () {
  ...
  function positionPanel(trigger, panel) {
    ...
    var left = Math.max(8, Math.min(r.left, vw - panelWidth - 8));
  ...
  function attachMenu() {
    ...
    if (!isDesktopLayout() && !isCoarsePointer()) return;
    ...
    document.addEventListener("keydown", function (e) {
      ...
      var links = panel.querySelectorAll('a[role="menuitem"]');
      if (e.key === "Escape" && panel.classList.contains(OPEN_CLASS)) {
        ...
        if (e.key === "ArrowDown") idx = idx < 0 ? 0 : (idx + 1) % links.length;
