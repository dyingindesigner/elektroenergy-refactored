...
(function () {
  ...
  var SCRIPT_VERSION = "2026-04-21-analytics-wave4";
  ...
  function pushEvent(event_type, path, meta) {
    ...
    var perEventSample = cfg.eventSampleRates && cfg.eventSampleRates[event_type];
    if (typeof perEventSample === "number" && perEventSample >= 0 && perEventSample < 1) {
      if (Math.random() > perEventSample) {
        droppedEvents += 1;
        return;
      }
    }
    QUEUE.push({
      ...
      path: path || pathNow(),
      meta: Object.assign({ script_version: SCRIPT_VERSION }, meta || {}),
    });
    scheduleFlush();
  }
  ...
  function flush(isUnload) {
    ...
    fetch(url, {
      ...
      // Return events back to the front of queue for retry.
  ...
  function handleLocation(reason) {
    ...
    var key = p + "|" + String(title).slice(0, 120);
    if (key !== lastPageviewKey) {
      lastPageviewKey = key;
      pushEvent("pageview", p, { reason: reason || "load", title: title });
    }
  ...
  var onClickTracked = core.throttle
    ? core.throttle(function (ev) {
        bumpActivity();
  ...
  /** --- SPA / zmeny URL bez polling intervalu --- */
  var lastHref = location.href;
  function trackUrlChange(reason) {
    var href = location.href;
    if (href === lastHref) return;
    lastHref = href;
    handleLocation(reason || "url_change");
  }

  window.addEventListener("popstate", function () {
    trackUrlChange("popstate");
  });
  window.addEventListener("hashchange", function () {
    trackUrlChange("hashchange");
  });
