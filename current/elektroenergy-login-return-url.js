...
(function () {
  ...
  var redirectedInThisPage = false;
  var DEBUG = !!window.EE_LOGIN_RETURN_DEBUG;
  function log() {
    if (!DEBUG) return;
    var args = Array.prototype.slice.call(arguments);
    args.unshift("[ee-login-return]");
    try {
  ...
  function writeStoredUrl(key, url, reason) {
    try {
  ...
  function rememberLastPublicPage() {
    ...
    writeStoredUrl(STORAGE_LAST_PUBLIC_KEY, current, "public_page");
  }

  function getLastPublicCandidateUrl() {
    var last = readStoredUrl(STORAGE_LAST_PUBLIC_KEY);
    if (!last || !last.url) return null;
    return isAllowedReturnUrl(last.url, { allowClientRoot: false }) ? last.url : null;
  }
  ...
  function rememberLoginReturnUrl(form) {
    ...
    writeStoredUrl(STORAGE_LOGIN_KEY, candidate, "login_submit");
    log("remember login return", candidate);
  }
  ...
  function tryRedirectAfterLoginLanding() {
    ...
    var payload = readStoredUrl(STORAGE_LOGIN_KEY);
    ...
    if (redirectedInThisPage) return;
    redirectedInThisPage = true;
    log("redirect after login", target, payload);
  ...
  function rememberLogoutReturnUrl() {
    var current = window.location.href;
    if (!isAllowedReturnUrl(current, { allowClientRoot: true })) return;
    writeStoredUrl(STORAGE_LOGOUT_KEY, current, "logout_click");
    log("remember logout return", current);
  }
  ...
  function tryRedirectAfterLogoutLanding() {
    ...
    var payload = readStoredUrl(STORAGE_LOGOUT_KEY);
    var target = payload && payload.url;
    ...
    if (redirectedInThisPage) return;
    redirectedInThisPage = true;
    log("redirect after logout", target, payload);
    window.location.replace(target);
  }
